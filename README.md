...
The assistant immediately shows your sent message and a pending reply. Compatible models stream their reply into the conversation as they write; waiting, thinking, source-tool work, and elapsed time stay visible. No manual refresh is needed. You can draft your next message while receiving a reply, stop it from the conversation, or scroll back without being pulled to the bottom (Jump to latest reply returns you). Reloading or reconnecting restores the current conversation and live reply without resending the prompt. If the provider returns a complete JSON response instead of a stream, the pending indicator remains until it arrives. Stopped or interrupted replies retain received text and show their status. The activity banner also shows the planned output allowance. Inspect result shows applied reasoning, requested/effective output limits, estimated input, and actual usage reported by the server for every call. Completed failures stay visible, with inspection and retry controls. If a model exhausts its output limit before producing text, choose Automatic, increase **Output tokens**, reduce reasoning in **Model settings**, or change the server limit when using Unlimited, then retry. Empty/whitespace replies, blocked output, malformed tool calls, and detectable violations of reasoning Off remain visible failures with the response retained. Discussion sessions reply in the conversation; source-edit sessions stage changes for review and application. The HTTP server prints timestamped activity and error messages to stderr. Examples include “Writing prose,” “Checking story consistency,” “Searching project references,” and “Refreshing the model list.” Errors include an operation/run reference and a concise reason (such as an output limit or API HTTP status). Prompts, story text, API keys, and provider response bodies stay out of the console. Routine state polling and draft autosaves are quiet. Redirect stderr to retain the log, for example `iterauthor 2>>iterauthor.log`. The optional TUI leaves these console messages disabled so they do not overwrite its screen.
...
This is a single-author experiment. Dependency scheduling, automatic multi-level outline expansion, resumable queues and native provider-specific inference APIs remain future work. Context summaries are inspectable history; this build does not reuse them to skip calls. Reviewer approval is a model judgment, not proof of novel-wide continuity.
