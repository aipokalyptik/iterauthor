...
`make test` passed: 41 tests across six packages, including the race detector, followed by `go vet ./...`. Each package has a 60-second test timeout; UI waits and fake network startup have shorter bounds.
