...
## Generation requires an author request

A regression opens a persisted project with the old `generate_after_editing: true` setting. Source/config saves, Reload, invalidation, opening a conversation, saving an unsent message, changing its model, and the legacy Finish editing API call produce zero inference calls and no runs. An explicit branch-generation request then completes only that branch. Subsequent edits, prose revisit decisions, and finishing editing do not regenerate it. Context selection/review calls remain part of explicitly requested work; catalog discovery performs no inference.
