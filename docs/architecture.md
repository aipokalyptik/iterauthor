...
The optional `model.StreamingClient` reports public text deltas and reasoning/tool activity signals. HTTP uses a single streaming Chat Completions request, assembling fragmented tool calls and usage before normal response validation. A server returning ordinary JSON is accepted without retrying the request. The engine publishes stage/model/phase activity; conversation text alone becomes a live preview. Reasoning and context/review output stay out of that preview. The service holds it in memory in the detached `Work` view, coalesces notifications to about ten per second (plus phase changes), and persists final replies once. Browser disconnects do not cancel inference; reloading obtains the same operation and current preview. Stop and provider disconnect retain partial output, and retries always require an explicit command. Streaming does not make queues resumable after a server restart.
