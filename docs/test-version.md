...
Projects start idle. Saving, reloading, importing/applying proposals, invalidating prose, and finishing editing never schedule model work. The legacy `generate_after_editing` field is retained for file compatibility but ignored. Generation requires an explicit draft/regenerate, outline-development, assistant Send, or Test model request. Requested branches/whole-story queues and their bounded review loops complete within that request. Automatic context selection runs only inside requested work; catalog refreshes read metadata without inference.
